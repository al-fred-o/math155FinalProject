...
Next we plot the model's predictions for the future. The model’s forecasts are slightly higher than the actual values, however, these appear to be within forecast limits, so while it is noticable, it is not statistically significant.
...
Beginning with the seasonally differenced data, the top 2 row of the plots suggest two further strong candidate models (models 2 and 3), an $\ARIMA(2,0,0) \times (1,1,0)_{12}$ and $\ARIMA(1,0,0) \times (0,1,1)_{12}$.
...
Finally, we check the extended auto correlation function of each series to evaluate if these models appear in the EACF charts as well.

```{r}
eacf(cRev_diff01, ar.max = 13, ma.max = 13)
eacf(cRev_diff11, ar.max = 13, ma.max = 13)
```

TODO: write about EACF

\subsection{Model Diagnostics}
We now define our 4 models and evaluate them.
```{r, echo = FALSE}
...
The runs test p-values on the 4 models are:
```{r}
runs_tests = vector(mode = "numeric", length = 4)
...
We see that models 1, 2, and 3 all have p-values above 0.05 indicating that they pass the runs test.
However, model 3 has a significantly larger p-value than the first two indicating it more comfortably passes the test.
Model 4 does not pass the test, and a more detailed look at the ouput shows that it has only 54 observed runs compared to 67.3 expected runs.
This indicates that the residuals from model 4 still are positively correlated.

Moving on to the Wilkes-Shapiro test, this tests the models residuals for normality.
The p-values of the 4 models are 
```{r}
shapiro_tests = vector(mode = "numeric", length = 4)
...
We will use a lag of 15 since we models that don't capture seasonal effects correctly on this dataset may exhibit significant autocorrelations at lags of 13 - 15.
The p-values from the Ljung-Box test are
```{r}
LB_tests = vector(mode = "numeric", length = 4)
