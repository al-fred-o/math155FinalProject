...
plot_months(cRev, main = "Total Revenue (1990-2020)", ylab = "Revenue (Millions)", xlab = "Year")
```

```{r}
totalRev2 = window(cRev, start = c(2008, 1))
...
```{r}
model1 = arima(recRev, order = c(2,1,0), seasonal = list(order=c(0,1,1),period= 12))
model2 = arima(recRev, order = c(2,1,1), seasonal = list(order=c(0,1,1),period= 12))

model3 = arima(recRev, order = c(2,1,0), seasonal = list(order=c(1,1,1),period= 12))
model4 = arima(recRev, order = c(2,1,1), seasonal = list(order=c(1,1,1),period= 12))
...
```{r}
num = length(actual)
res = plot(model,n.ahead=num, n1 = c(2016,2), col = "red", main = "ARIMA(,,)x(,,)_12 Forecast", ylab ="Revenue", xlab = "Year")
lines(actual,pch=3) 
